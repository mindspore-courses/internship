...
class RealDexReader:
  def __init__(self, video_dir, downscale=1, shorter_side=None, zfar=np.inf) :
    ...
    self.zfar = zfar
    self.color_files = sorted(glob.glob(f"{self.video_dir}/rgb/*.jpg"), key=natural_sort_key)
  ...
  def get_depth(self,i):
    depth = cv2.imread(self.color_files[i].replace('rgb','depth').replace(".jpg", ".png"),-1)/1e3
    depth = cv2.resize(depth, (self.W,self.H), interpolation=cv2.INTER_NEAREST)
    depth[(depth<0.1) | (depth>=self.zfar)] = 0
    return depth

  def get_mask(self,i):
    mask = cv2.imread(self.color_files[i].replace('rgb','masks').replace(".jpg", ".png"),-1)
    if len(mask.shape)==3:
      for c in range(3):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
