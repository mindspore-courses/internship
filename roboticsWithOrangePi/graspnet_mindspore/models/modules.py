...
class ApproachNet(nn.Module):
    ...
    def forward(self, seed_xyz, seed_features, end_points):
        ...
        top_view_scores, top_view_inds = P.max(view_score, dim=2) # (B, num_seed)
        ...
        vp_xyz = P.gather_elements(template_views, 2, top_view_inds_).squeeze(2) #(B, num_seed, 3)
        vp_xyz_ = vp_xyz.view(-1, 3)
        batch_angle = P.zeros(vp_xyz_.size(0), dtype=vp_xyz.dtype, device=vp_xyz.device)
