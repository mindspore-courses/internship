...
from graspnet_dataset import GraspNetDataset, load_grasp_labels
...
def log_string(out_str):
    ...
    print(out_str)
#content init
context.set_context(mode=mindspore.context.PYNATIVE_MODE, device_target="Ascend", device_id=0)
...
TRAIN_DATALOADER = ds.GeneratorDataset(TRAIN_DATASET, column_names=["data"], shuffle=True)
TEST_DATALOADER = ds.GeneratorDataset(TEST_DATASET, column_names=["data"], shuffle=False)
print(len(TRAIN_DATALOADER), len(TEST_DATALOADER))

TRAIN_DATALOADER = TRAIN_DATALOADER.batch(batch_size=cfgs.batch_size, drop_remainder=True)
TEST_DATALOADER = TEST_DATALOADER.batch(batch_size=cfgs.batch_size, drop_remainder=True)
# Init the model and optimzier
net = GraspNet(input_feature_dim=0, num_view=cfgs.num_view, num_angle=12, num_depth=4,
                        cylinder_radius=0.05, hmin=-0.02, hmax_list=[0.01,0.02,0.03,0.04])

# Load the Adam optimizer
optimizer = nn.AdamWeightDecay(net.parameters(), learning_rate=cfgs.learning_rate, beta1=0.9, beta2=0.999,eps=1e-8,weight_decay=cfgs.weight_decay)
# Load checkpoint if there is any
it = -1 # for the initialize value of `LambdaLR` and `BNMomentumScheduler`
start_epoch = 0
if CHECKPOINT_PATH is not None and os.path.isfile(CHECKPOINT_PATH):
    checkpoint = load_checkpoint(CHECKPOINT_PATH)
    load_param_into_net(net, checkpoint['model_state_dict'])
            ...
            if 'list' in key:
                for i in range(len(batch_data_label[key])):
                    for j in range(len(batch_data_label[key][i])):
                        batch_data_label[key][i][j] = Tensor(batch_data_label[key][i][j])
            else:
                batch_data_label[key] = Tensor(batch_data_label[key])
